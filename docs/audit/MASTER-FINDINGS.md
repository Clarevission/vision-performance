...
| F-16 | P2 | Content | Phone number +1 (780) 886-4397 was published but not confirmed as monitored | Owner decision log | RESOLVED | Owner decision (25 Sep): removed from the footer, contact page, legal pages and Organization JSON-LD. A test blocks `tel:` links until the owner re-adds a staffed number |
| F-17 | P2 | Privacy | The privacy policy named a "privacy contact" but no accountable individual, which PIPA expects the organization to designate | `privacy.html` | RESOLVED | Owner decision: a designated Privacy Officer (by title), reached at info@ with "Privacy request" in the subject. Optional: create a `privacy@` alias and point the policy at it |
...
| F-19 | P3 | Content / IA | The "Client sign-in" footer link implied existing portal clients | `footer.html` | RESOLVED | Owner decision: link removed; `/portal` stays reachable by direct URL for onboarded clients |
...
| F-27 | P3 | Analytics | `vpiTrack` pushes events to `dataLayer` only; no analytics provider is connected, so conversions are not measured | `site.js` | REQUIRES EXTERNAL CREDENTIAL | Owner chose Cloudflare Web Analytics. The code is ready (`lib/analytics.js`): setting `CF_ANALYTICS_TOKEN` in Render turns on the beacon, the CSP allowances and the privacy-policy paragraph together. Upgrade to Plausible later for custom events |
...
| F-32 | P2 | Privacy | The privacy policy listed Render, Neon and Resend but not Cloudflare, which proxies every request and sees visitors' IP addresses | `privacy.html` | RESOLVED | Cloudflare named as the content delivery and security provider |
